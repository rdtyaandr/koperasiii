...
<head>
    ...
    <style>
        ...
        .edit-form {
            ...
            position: fixed;
            top: 0;
            right: -430px; /* Mulai dari luar tampilan */
            width: 430px;  /* Memperbesar lebar form */
            padding: 20px;
            border-left: 1px solid #ddd;
            background-color: #fff;
            z-index: 1000;
            box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); /* Menambahkan bayangan */
            transition: right 0.5s ease; /* Animasi slide */
        }
        .container {
            display: flex;
            position: relative;
        ...
        .profile-card-container {
            flex: 1; /* Profil card mengambil sisa ruang */
        }
        .edit-form-container {
            flex: 0 0 430px; /* Lebar tetap untuk form edit */
        }
        .btn-edit {
            margin-top: 20px;
        }
        ...
        .edit-form .input-field label {
            width: 100%; /* Memperbesar lebar input */
        }
        .edit-form .btn {
            margin-right: 10px;
        }
                ...
                <div class="col s12 m8 l8">
                    <div id="profile-card" class="card">
                        ...
                        <div class="card-content">
                            <img src="<?= base_url('assets/upload/dokumen/' . $pengguna['pengguna_picture']) ?>" alt="" class="circle responsive-img activator card-profile-image">
                            <span class="card-title activator grey-text text-darken-4"><?= $pengguna['nama_lengkap'] ?></span>
                            <p class="grey-text"><i class="mdi-action-perm-identity"></i> <?= $pengguna['username'] ?></p>
                            ...
                            <p class="grey-text"><i class="mdi-action-date-range"></i> Bergabung sejak: <?= date('d-m-Y', strtotime($pengguna['pengguna_date_created'])) ?></p>
                            <p class="grey-text"><i class="mdi-action-date-range"></i> Diperbarui Pada: <?= date('d-m-Y', strtotime($pengguna['pengguna_date_update'])) ?></p>
                            <button class="btn waves-effect waves-light btn-edit" onclick="toggleEditForm()">Edit</button>
                        </div>
                    </div>
                </div>
            </div>
